...
Surveys such as the Sloan Digital Sky Survey \cite{sdss} and most recently the Dark Energy Spectroscopic Instrument \cite{desi} have spectroscopically observed millions of galaxies, producing detailed maps of a large volume of the observable universe.
Current and upcoming surveys such as Euclid \citep{euclid} and the Roman Observatory \cite{roman} will deliver more and more high quality galaxy clustering data sets.

Standard approaches to analyzing this spectroscopic survey data rely on analytic techniques to model summary statistics of the galaxy field , and Monte Carlo techniques to sample the parameters of interest \cite{sdss_pk, desi_pk}.
...
For instance, the DESI analyses use a perturbation theory approach, and their analysis is limited to $\kmax=XXX$ for the power spectrum and $\kmax=XXX$ for the bispectrum. 
...
\cite{santi_robust_2023} employ the 1000-simulation Latin hypercubes of the CAMELS suite \citep{camels}, which are hydrodynamical simulations with $(25 \Mpch)^3$ volumes, to perform SBI using graph neural networks; they find that their inference on $\om$ is robust to galaxy formation physics.
...
The emulator \mtm \citep{jamieson_field_2023} uses a convolutional neural network-based architecture to map from Zel'dovich approximation displacements to $N$-body displacements, trained on the Quijote simulation suite \cite{quijote}.
It has been used in a range of applications, such as producing large-volume emulated simulations \citep{scoggins2502.13242}; variants of \mtm have been designed to emulate modified gravity \citep{saadeh2406.03374} and to learn the mapping from non-linear structures to the initial conditions of the universe \citep{doeser_bayesian_2024}.
There has been continued work on developing full-field emulators, with a range of training data and neural architectures \citep{kang2603.18165, ullmo2403.02171, jacobson2607.27320}.


Robustly the galaxy--dark matter connection is a key challenge in cosmological inference.
Halo-based models such as the halo occupation distribution \citep{HOD} populate dark matter halos with galaxies based on halo mass and properties, but their flexibility degrades their robustness, particularly for higher-order statistics (e.g. \citealt{contreras2407.18912, wang2026}).
More physically motivated models like subhalo-abundance matching \citep{sham, shame} are more robust, but require merger trees which are more computationally expensive; even more complicated models like semi-analytic models (e.g. \citep{sams}) and hydrodynamical simulations (e.g. \citep{hydro}) are both expensive and make specific choices regarding galaxy formation, which makes robustness a challenge.
EFTofLSS offers a perturbative approach that is robust on large scales, but breaks down on small scales \citep{eft}.

The hybrid Lagrangian bias expansion (or hybrid effective field theory) offers an approach that is accurate into the small-scale regime while remaining computationally relatively low-cost.
...
This approach has been shown to be effective down to $k\sim0.6$ \citep{Modi2020, pellejero-ibanez_modelling_2022}, and has been used to construct accurate emulators of the power spectrum \citep{zennaro2021, kokron2021, pellejero_ibanez_hybrid-bias_2023}.
...
They found 1-2\% precision on the power spectrum $k\sim0.6$; while there were some biases at intermediate scales, these were absorbed by  nuisance parameters descrbing stochasticity.
...
[how many sims paper]
Based on their ..., \cite{tucci_eftoflss_2024} estimate that of order $10^5$ simulations are required ...
...
We thus turn to a field-level emulator of the matter distribution, \mtm \cite{jamieson_field_2023}.
...
We need to define the range of the bias coefficients $\{b\}$ we use to sum the fields; for this, we use the results of \cite{zennaro_priors_2022}, which define priors that capture a range of realistic galaxy populations modeled with the extended subhalo abundance matching technique \cite{shame}.
...
This was achieved with the \cite{qian_nested_2009} algorithm for nested latin hypercubes, which works by \dots \ksf{TODO}.
...
Our final test set is our \emph{SHAMe mock}; we construct a realistic catalog of galaxies using the extended subhalo abundance matching (SHAMe) approach \cite{Contreras2020} applied to a \bacco $N$-body simulation \cite{Angulo2021}.
...
In detail, we use a simulation with a box size of (1024 $\hinv$Mpc)$^3$ (slightly larger than the \mmocks training data) and $3072^3$ particles, which uses the fixed-and-paired technique \citep{Angulo2021}, and has Planck cosmology $\Omega_\mathrm{c}=0.309$, $\sig=0.816$, $\h=0.677$, $\ob=0.0486$, and $n_\mathrm{s}=0.967$ \citep{Planck2014a}.
Halos are identified using a variant of the \textsc{SUBFIND} halo finder \citep{Springel2001}, and we populate these halos with galaxies using the SHAMe model with the following parameter choices: $\sigma_\mathrm{logM}=0.47513517$, $t_\mathrm{merger}=-0.4577029$, $f_\mathrm{k,cen+sat}=0.20147321$, $f_\mathrm{k,cen-sat}=-0.02964998$, $\beta_\mathrm{lum}=14.82600407$.
...
For the SHAMe data, we obtain a XXX\% improvement in precision on $\om$ and XXX\% on $\sig$ when including $\pgm$ compared to $\pk$ alone; additionally including $\bispec$ results in XXX\% and XXX\% improvements on $\om$ and $\sig$ respectively compared to $\pk+\pgm$.

We do see slight biases in the values of $\bo$ and $\bt$ when using the full statistics combination, at the XXX$\sigma$ and XXX$\sigma$ levels, while these were on average accurately constrainted in the cosmic variance test set.
...
\caption{Impact of noise on the inferred posteriors, for the mean of $\pk+\bispec+\pgm$ over the cosmic variance test set. The brown contours show inference trained on statistics computed on noiseless realizations, and inferred with a model with no noise; the pink contours show inference computed on realizations injected with the \citet{rubira_non-gaussian_2025} noise model, and inferred with the same noise model. Even with perfect knowledge of the noise model, the constraints on $\sig$ and $\bo$ degrade significantly.}
% \ksf{could be interesting to show what happens w denser model. complement shame at diff number densities - add that plot! did lrg-like same, but may make sense to have a much denser sample - more to gain.} \ksf{esp bc limited by noise model for pgm}
...
(Previous works using the hybrid bias expansion with a power spectrum emulator find accuracy down to $\kmax=0.6\,\hMpc$ \citep{hadzhiyska_hefty_2021, zennaro2021}.)
...
\caption{Convergence of the inference with training set size, for $\pk+\bispec+\pgm$ on the coverage test set for the three key parameters $\om$, $\sig$, and $\bo$ combined. The top panel shows the figure of merit (FoM), and the bottom panel shows the figure of bias (FoB), normalized by the 68th percentile of the $\chi^2$ distribution for three dimensions (corresponding to the three parameters); the median values over the coverage set are shown. The opacity indicates the number of training cosmologies. Increasing the number of training points increases the precision signficantly until $N_\text{train}=\,\sim$20,000 and then more marginally, with the number of cosmologies and bias parameter sets contributing similar precision; the bias also plateaus around $N_\text{train}=\,\sim$20,000, with the number of cosmologies mattering more than the number of bias parameters for smaller $N_\text{train}$. \ksf{make higher res}}
...
In this work, large language models (LLMs) were used to make code modifications, write basic code such as plotting functions, support background research, and provide basic \LaTeX and citation support.
Core code and all writing did not use LLMs.
