...
The results of this analysis outline the successes and challenges for SBI with the hybrid Lagrangian bias expansion and emulated $N$-body displacements.
...
We have used Lagrangian bias expansion to second order; some works have shown that a partial third order expansion is important \citep{XXX} ... \ksf{expand}
We have chosen a specific noise model, of a Gaussian field coupled to the bias fields following \cite{rubira_non-gaussian_2025}, to first order in the noise field.
This does not capture the noise in $\pgm$; extending this model to higher order (at the expense of adding additional parameters) or testing other noise models may be important for ensuring the most robust inference.

The smallest scales we can include are currently limited by... \ksf{expand on this}
...
This stands somewhat in contrast to our finding that even 320,000 total training data points does not seem to saturate in precision, and that 2,000 cosmologies provide comparable results to 10,000 given an equal total number of simulations including multiple galaxy bias parameter sets.
However, our convergence findings are not directly comparable, as this work analyzes a the matter field statistic compared to our galaxy field statistics, and goes to higher $k$.
...
\ksf{TODO: also discuss tucci and schmidt paper which explores convergence}
...
The model uses the hybrid Lagrangian bias expansion to second order, based on emulated particle and velocity displacement fields from the \mtm emulator trained on the Quijote mocks, to generate sets of bias fields that can be weighted and summed to produce tracer fields.
We also include a full-field noise model based on a Gaussian field coupled to the bias fields, which we find to be critical for unbiased inference.
